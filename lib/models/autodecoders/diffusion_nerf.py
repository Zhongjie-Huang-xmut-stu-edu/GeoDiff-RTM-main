...
from mmgen.models.builder import MODELS, build_module, MODULES
...
class DiffusionNeRF(MultiSceneNeRF):
    ...
    def __init__(self,
        ...
        if edge_loss is not None:
            self.edge_loss = MODULES.build(edge_loss)
        else:
            self.edge_loss = None
        ...
        if perceptual_loss is not None:
            self.perceptual_loss = MODULES.build(perceptual_loss)
        else:
            self.perceptual_loss = None
            
        if normal_loss is not None:
            self.normal_loss = MODULES.build(normal_loss)
        else:
    ...
    def format_rays_to_image(self, rays_rgb, img_size=64):
        
        if rays_rgb.dim() == 2:  # [N_rays, 3]
            N_rays = rays_rgb.size(0)
            H = W = int(N_rays ** 0.5) if int(N_rays ** 0.5) ** 2 == N_rays else img_size
            try:
                img = rays_rgb.view(H, W, 3).permute(2, 0, 1).unsqueeze(0)  # [1, 3, H, W]
            except:
                
                img = torch.zeros(1, 3, img_size, img_size, device=rays_rgb.device)
                valid_pixels = min(N_rays, img_size * img_size)
                img.view(1, 3, -1)[:, :, :valid_pixels] = rays_rgb[:valid_pixels].T.unsqueeze(0)
                
        elif rays_rgb.dim() == 3:  # [B, N_rays, 3]
            B, N_rays, _ = rays_rgb.shape
            H = W = int(N_rays ** 0.5) if int(N_rays ** 0.5) ** 2 == N_rays else img_size
            try:
                img = rays_rgb.view(B, H, W, 3).permute(0, 3, 1, 2)  # [B, 3, H, W]
            except:
                
                img = torch.zeros(B, 3, img_size, img_size, device=rays_rgb.device)
                valid_pixels = min(N_rays, img_size * img_size)
                img.view(B, 3, -1)[:, :, :valid_pixels] = rays_rgb[:, :valid_pixels].permute(0, 2, 1)
        else:
        ...
        if 'decoder' in optimizer or len(code_optimizers) > 0:
            ...
            if hasattr(self, 'perceptual_loss') and self.perceptual_loss is not None:
                try:
                    ...
                    pred_img = self.format_rays_to_image(out_rgbs)
                    target_img = self.format_rays_to_image(target_rgbs)

                    perceptual_loss_value = self.perceptual_loss(pred_img, target_img)
                    loss_decoder = loss_decoder + perceptual_loss_value
                    log_vars_decoder['loss_perceptual'] = float(perceptual_loss_value)
                except Exception as e:
                    print(f"Perceptual loss computation failed: {e}")
                    log_vars_decoder['loss_perceptual'] = 0.0

           
            if hasattr(self, 'normal_loss') and self.normal_loss is not None:
                try:
                    
                    pred_img = self.format_rays_to_image(out_rgbs)
                    target_img = self.format_rays_to_image(target_rgbs)

                    normal_loss_value = self.normal_loss(pred_img, target_img)
                    loss_decoder = loss_decoder + normal_loss_value
                    log_vars_decoder['loss_normal'] = float(normal_loss_value)
                except Exception as e:
                    print(f"Normal loss computation failed: {e}")
                    log_vars_decoder['loss_normal'] = 0.0
            
            
            if hasattr(self, 'edge_loss') and self.edge_loss is not None:
                try:
                   
                    pred_img = self.format_rays_to_image(out_rgbs)
                    target_img = self.format_rays_to_image(target_rgbs)

                    edge_loss_value = self.edge_loss(pred_img, target_img)
                    loss_decoder = loss_decoder + edge_loss_value
                    log_vars_decoder['loss_edge'] = float(edge_loss_value)
                except Exception as e:
                    print(f"Edge loss computation failed: {e}")
                    log_vars_decoder['loss_edge'] = 0.0
    ...
    def val_step(self, data, viz_dir=None, viz_dir_guide=None, **kwargs):
        ...
        with torch.no_grad():
            ...
            if 'test_poses' in data:
                ...
                if hasattr(self, 'edge_loss') and self.edge_loss is not None and pred_imgs is not None:
                    try:
                        with torch.no_grad():
                            if 'test_imgs' in data and data['test_imgs'] is not None:
                                target_imgs = data['test_imgs']

                                
                                if pred_imgs.dim() == 5:
                                    if pred_imgs.shape[2] == 3:  
                                        B, N_imgs, C, H, W = pred_imgs.shape
                                        pred_formatted = pred_imgs.view(B * N_imgs, C, H, W)
                                    else:  
                                        B, N_imgs, H, W, C = pred_imgs.shape
                                        pred_formatted = pred_imgs.permute(0, 1, 4, 2, 3).contiguous()
                                        pred_formatted = pred_formatted.view(B * N_imgs, C, H, W)
                                elif pred_imgs.dim() == 4 and pred_imgs.shape[-1] == 3:  # [B, H, W, 3]
                                    pred_formatted = pred_imgs.permute(0, 3, 1, 2)  # [B, 3, H, W]
                                else:
                                    pred_formatted = pred_imgs

                                if target_imgs.dim() == 5:
                                    if target_imgs.shape[2] == 3:  
                                        B, N_imgs, C, H, W = target_imgs.shape
                                        target_formatted = target_imgs.view(B * N_imgs, C, H, W)
                                    else:  
                                        B, N_imgs, H, W, C = target_imgs.shape
                                        target_formatted = target_imgs.permute(0, 1, 4, 2, 3).contiguous()
                                        target_formatted = target_formatted.view(B * N_imgs, C, H, W)
                                elif target_imgs.dim() == 4 and target_imgs.shape[-1] == 3:  # [B, H, W, 3]
                                    target_formatted = target_imgs.permute(0, 3, 1, 2)  # [B, 3, H, W]
                                else:
                                    target_formatted = target_imgs

                                
                                batch_size = 128  
                                total_samples = pred_formatted.shape[0]
                                edge_losses = []

                                for i in range(0, total_samples, batch_size):
                                    end_idx = min(i + batch_size, total_samples)
                                    pred_batch = pred_formatted[i:end_idx]
                                    target_batch = target_formatted[i:end_idx]

                                    batch_loss = self.edge_loss(pred_batch, target_batch)
                                    edge_losses.append(batch_loss)

                                val_edge = torch.stack(edge_losses).mean()
                                log_vars['val_edge'] = float(val_edge)
                    except Exception as e:
                        print(f"Validation edge loss failed: {e}")
                        log_vars['val_edge'] = 0.0
                ...
                if hasattr(self, 'perceptual_loss') and self.perceptual_loss is not None and pred_imgs is not None:
                    try:
                        with torch.no_grad():
                            if 'test_imgs' in data and data['test_imgs'] is not None:
                                target_imgs = data['test_imgs']
                                ...
                                if pred_imgs.dim() == 5:
                                    if pred_imgs.shape[2] == 3:  
                                        B, N_imgs, C, H, W = pred_imgs.shape
                                        pred_formatted = pred_imgs.view(B * N_imgs, C, H, W)  # [972, 3, 128, 128]
                                    else:  
                                        B, N_imgs, H, W, C = pred_imgs.shape
                                        pred_formatted = pred_imgs.permute(0, 1, 4, 2, 3).contiguous()
                                        pred_formatted = pred_formatted.view(B * N_imgs, C, H, W)
                                ...
                                for i in range(0, total_samples, batch_size):
                                    ...
                                    batch_loss = self.perceptual_loss(pred_batch, target_batch)
                                    perceptual_losses.append(batch_loss)
                                
                                val_perceptual = torch.stack(perceptual_losses).mean()
                                log_vars['val_perceptual'] = float(val_perceptual)
                    except Exception as e:
                        print(f"Validation perceptual loss failed: {e}")
                        log_vars['val_perceptual'] = 0.0

                if hasattr(self, 'normal_loss') and self.normal_loss is not None and pred_imgs is not None:
                    try:
                        with torch.no_grad():
                            if 'test_imgs' in data and data['test_imgs'] is not None:
                                target_imgs = data['test_imgs']
                                ...
                                for i in range(0, total_samples, batch_size):
                                    ...
                                    batch_loss = self.normal_loss(pred_batch, target_batch)
                                    normal_losses.append(batch_loss)
                                
                                val_normal = torch.stack(normal_losses).mean()
                                log_vars['val_normal'] = float(val_normal)
                    except Exception as e:
                        print(f"Validation normal loss failed: {e}")
                        log_vars['val_normal'] = 0.0
        ...
        if save_dir is not None:
            ...
            if save_mesh:
                ...
                self.save_mesh(save_dir, decoder_module, code, data['scene_name'], mesh_resolution, mesh_threshold)
